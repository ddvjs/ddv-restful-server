...
function request (urlInputs, body, method, contentType) {
  ...
  if (Array.isArray(urlInputs)) {
    ...
    if (!urlInput) {
      return Promise.reject(new Error('Parameter "urlInput" is not a valid parameter'))
    }
  } else if (typeof urlInputs === 'object') {
    opt = urlInputs
  } else {
    urlInput = urlInputs
  }
  ...
  requestfn = (opt.protocol === 'https:' ? https : http).request
